...
char_dim, char2int, int2char, X, yd = make_char_embedding(["hello x", "goobye", "whaver dude"])
...
def cost_over_data(X,y):
    cost = 0.0
    for i in range(0,X.shape[0]):
        x_e = X[i_x,:].astype(int)
        y_e = yd[i_x,:].astype(int)
        cost += run_cost(x_e,y_e,u_train,v_train,w_train,init_s_dummy)
